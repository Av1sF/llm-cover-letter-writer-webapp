...
def testHomeInvalidMethod(client):
    """
    ...
    assert b"<title>Cover Letter Writer. </title>" not in response.data
...
def testRegisterInvalidForm(client):
    ...
    WHEN a new user is wants to register and submits data 
...
def testLoginInvalidForm(client):
    ...
    GIVEN a taken invalid form from client 
    WHEN a  user is wants to login and submits data 
    THEN check that response states that there is a form error 
    """
...
def testLoginInvalidUsername(client):
    ...
    GIVEN a taken non-registered username from client 
    WHEN a user is wants to login and submits data 
...
def testLoginInvalidPassword(client):
    ...
    THEN check that response states that the "Username or password is not correct"
    """
    response = client.post("auth/login", data={"username":"testing123", 
...
def testValidLogin(client): 
    ...
    WHEN a user is wants to login and submits data 
    THEN check that response is a redirect and it returns the access token and csrf token 
    """
...
def testValidLogout(client):
    ...
    THEN check that response is a redirect and any tokens are expired  
    """
...
def testUnautUserUpdate(client):
    """
    Erroneous Data 
...
def testInvalidFormQueryModel(client):
    ...
    # form with right number of fields but wrong json fields names  
    response = client.post("model/query", 
        headers= {"X-CSRF-TOKEN": csrfAccessToken},
        data={
    ...
    # form with all valid fields but with extra fields 
    response = client.post("model/query", 
        headers= {"X-CSRF-TOKEN": csrfAccessToken},
        data={
        "Job Title" : "Data Scientist", 
